...
def execute_algorithm(monfg, u_tpl, algorithm='PSNE', variant='alternating', iterations=1000, seed=None):
    ...
    Args:
        ...
        variant (str, optional): The variant to use when executing fictitious play or best-response iteration.
    ...
    if algorithm == 'PSNE':
        ...
        ne, final_strategy = iterated_best_response(monfg, u_tpl, max_iter=iterations, variant=variant, seed=seed)
        results = ne, final_strategy
    elif algorithm == 'FP':
        ne, final_strategy = fictitious_play(monfg, u_tpl, max_iter=iterations, variant=variant, seed=seed)
        results = ne, final_strategy
    else:
